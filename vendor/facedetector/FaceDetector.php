    ...
    public function toJpeg($fname = '')
    {
   	...
   	if(false) {
	       ...
	       imagerectangle(
		    ...
		    $this->face['x'] - $margin,
		    $this->face['y'] - $margin,
		    $this->face['x']+$this->face['w'] + $margin,
		    $this->face['y']+ $this->face['w'] + $margin,
		    $color
		);
   	} else {
   		$outWidth = $this->face['w'] + (2 * $margin);
    		$outCanvas = imagecreatetruecolor($outWidth, $outWidth);
           imagecopyresampled(
                $outCanvas,
                $this->canvas,
                0,
